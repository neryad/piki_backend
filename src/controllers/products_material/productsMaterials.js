...
export const getAllMaterialsProductRelation = async (req, res) => {
  try {
    const response = await turso.execute(
      " SELECT M.name as materialName,M.id as materialId,P.name as productName,P.id as productId, PM.quantityUsed as quantityUsed FROM 'productsMaterials' as PM INNER JOIN 'materials' as M on PM.material_id =  M.id INNER JOIN 'products' as P on PM.product_id =  P.id  "
    );
    const materialsProducts = response.rows;
    res.status(200).json(materialsProducts);
  } catch (error) {
